...
public class repositorio {
    ...
    public static ArrayList<Datos_usuarios> categoriainfantilpun() {
        ...
        try {
            String query = "SELECT * FROM usuario ORDER BY puntaje DESC LIMIT 0,3;";
            PreparedStatement sentenciaP = database.open().prepareStatement(query);
            ResultSet resultado = sentenciaP.executeQuery();
       ...
       public void validacion(int cedula){
     ArrayList<Datos_usuarios> personas = this.obtenerTodos();
     Iterator<Datos_usuarios> it = personas.iterator();
        Datos_usuarios num;
        while (it.hasNext()){
            num = it.next();
            if(num.getCedula()==cedula){
     JOptionPane.showMessageDialog(null, "JUGADOR YA REGISTRADO", "Error", JOptionPane.ERROR_MESSAGE);
